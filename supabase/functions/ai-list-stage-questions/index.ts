...
Deno.serve(async (req) => {
  ...
  const { data, error } = await admin.from("training_stage_tests")
    .select("questions,total_score,pass_score,ai_generated_at")
    .eq("project_id", body.project_id).eq("stage", body.stage).maybeSingle();
  if (error) return jsonResponse({ error: error.message }, 500);
  if (!data) return jsonResponse({ ok: true, questions: [], total_score: 0, pass_score: 70 });
  ...
  return jsonResponse({ ok: true, questions: sanitized, total_score: data.total_score, pass_score: data.pass_score });
});