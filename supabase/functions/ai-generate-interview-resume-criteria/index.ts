...
Deno.serve(async (req) => {
  ...
  const projRes = await admin
    .from("projects")
    .select("role_name,vacancy_text,company_text,salary_terms,schedule_terms,custom_wiki,motivation_text,motivation_text_detail,tasks_activity_text,training_prof_text,company_id")
  ...
  if ((proj as any).company_id) {
    const { data: co } = await admin
      .from("companies")
      .select("name,description_text,products_text,mission_text,about_text,industry")
