...
РАСЧЕТ БАЛЛОВ: Всего вопросов ${body.questions.length}. Максимальная сумма за все вопросы ровно 100 баллов. Распределяй максимальный балл поровну между всеми вопросами (например, если 20 вопросов — каждый максимум 5 баллов, в сумме ровно 100). Поле score в элементах items должно быть от 0 до макс. балла за этот вопрос.
Затем дай итог: общая сумма баллов (total 0..100), краткое summary (2-3 предложения), массив strengths и gaps (по 2-4 пункта).
