...
Составь чек-лист из 20 проверочных вопросов для собеседования на вакансию.
...
Верни СТРОГО ${SCHEMA}`;
  ...
  // Register job for fallback (no RR is charged in this function).
  const idem = `interview_checklist:${body.project_id}`;
  const job = await createOrReuseAiJob({
    userId: user?.id || null,
    jobType: "interview_checklist",
    idempotencyKey: idem,
