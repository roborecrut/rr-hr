...
${templates ? `\nЭТАЛОНЫ ПО ПОЛЯМ для роли «${body.role_name ?? "—"}» (структура/тон — НЕ копировать дословно):\n${templates}\n` : ""}${company ? `\nДАННЫЕ УЖЕ ЗАРЕГИСТРИРОВАННОЙ КОМПАНИИ «${body.company_name ?? "—"}» (используй для согласованности):\n${company}\n` : ""}${file ? `\nТЕКСТ ДОКУМЕНТА ВАКАНСИИ (распознан из файла, до 5000 символов — это ГЛАВНЫЙ источник фактов о вакансии):\n"""\n${file}\n"""\n` : ""}${body.hint ? `\nДополнительный контекст: ${body.hint}\n` : ""}
