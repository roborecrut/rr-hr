...
function buildVacancyMessages(body: any, strict: boolean) {
  ...
  const templates = body.templates && typeof body.templates === "object"
    ? Object.entries(body.templates as Record<string, string>)
        .filter(([, v]) => typeof v === "string" && v.trim().length > 0)
        .map(([k, v]) => `--- ${k} ---\n${v}`)
        .join("\n\n")
    : "";
  const fieldsIn = (body.fields ?? {}) as Record<string, string>;
  const filledKeys = Object.entries(fieldsIn).filter(([, v]) => (v ?? "").toString().trim().length > 0).map(([k]) => k);
  const emptyKeys = Object.entries(fieldsIn).filter(([, v]) => (v ?? "").toString().trim().length === 0).map(([k]) => k);

  const system =
`Ты — старший HR-копирайтер. Тебе дают:
ВАЖНО: пиши строго на русском языке. Избегай англицизмов, кроме общеупотребительных профессиональных терминов и тех, что явно указал пользователь.

 • уже частично заполненные 15 полей вакансии (JSON, блок «ИСХОДНЫЕ ПОЛЯ»),
 • эталонные образцы оформления по каждому из этих 15 полей именно для этой роли (блок «ЭТАЛОНЫ ПО ПОЛЯМ» — это структура и тон, а не текст для копирования),
 • данные уже зарегистрированной компании (JSON, для согласованности по графику / мотивации / команде / системам),
 • опционально — распознанный текст из загруженного документа о вакансии (до 5000 символов, блок «ТЕКСТ ДОКУМЕНТА»).
...
3. ПРИОРИТЕТ ИСТОЧНИКОВ: (а) текст из документа → (б) уже заполненные пользователем поля → (в) данные компании → (г) эталоны по роли. Никаких выдуманных конкретных цифр, ссылок, ФИО, дат — только обобщённые формулировки.
4. Если поле у пользователя уже заполнено — сохрани смысл, только улучши формулировки, форматирование и стиль; не выдумывай новые сущности.
   Если поле у пользователя пусто — извлеки данные из «ТЕКСТА ДОКУМЕНТА» и/или «ДАННЫХ КОМПАНИИ», и оформи строго по формату из «ЭТАЛОНА» для этого поля.
   Если и в документе/компании информации нет — допиши обобщённый, но осмысленный контент по эталону для этой роли.
...
${VACANCY_EXAMPLE}`;
  ...
  const user =
`ВАЖНО: ниже не пример, а реальные данные текущей вакансии. Заполняй именно поля из блока «ИСХОДНЫЕ ПОЛЯ ВАКАНСИИ». Если они пустые — извлекай факты из блока «ТЕКСТ ДОКУМЕНТА ВАКАНСИИ».

Должность: ${body.role_name ?? "—"}
Компания: ${body.company_name ?? "—"}
Заполнено пользователем полей: ${filledKeys.length}/15${filledKeys.length ? ` (${filledKeys.join(", ")})` : ""}.
Пустые поля (нужно заполнить по приоритету источников): ${emptyKeys.length ? emptyKeys.join(", ") : "—"}.

${file ? `ТЕКСТ ДОКУМЕНТА ВАКАНСИИ (распознан из файла, ГЛАВНЫЙ источник фактов; заполни поля по нему, а не по демо-примерам):
"""
${file}
"""
` : "ТЕКСТ ДОКУМЕНТА ВАКАНСИИ: не передан. Используй заполненные поля и данные компании.\n"}

ИСХОДНЫЕ ПОЛЯ ВАКАНСИИ (что заполнил пользователь):
${JSON.stringify(body.fields ?? {}, null, 2)}
${templates ? `\nЭТАЛОНЫ ПО ПОЛЯМ для роли «${body.role_name ?? "—"}» (только структура/формат, НЕ источник фактов и НЕ копировать дословно):\n${templates}\n` : ""}${company ? `\nДАННЫЕ ЗАРЕГИСТРИРОВАННОЙ КОМПАНИИ «${body.company_name ?? "—"}» (используй для согласованности, но не заменяй факты вакансии из документа):\n${company}\n` : ""}${body.hint ? `\nДополнительный контекст: ${body.hint}\n` : ""}
