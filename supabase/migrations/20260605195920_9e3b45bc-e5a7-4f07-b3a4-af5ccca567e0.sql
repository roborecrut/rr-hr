...
-- 1) Replace the unique-by-email constraint with unique per (email, project)
DROP INDEX IF EXISTS public.candidates_email_unique;
CREATE UNIQUE INDEX IF NOT EXISTS candidates_email_project_uidx
  ON public.candidates (lower(email), project_id)
  WHERE email IS NOT NULL AND project_id IS NOT NULL;

-- 2) candidate_email_signup: allow same email for different vacancies if password matches
...
BEGIN
  ...
  -- If already applied to this exact vacancy, just log them in (if password matches)
  ...
  -- Reuse password hash if email exists in other applications
...
-- 3) candidate_email_login: pick a specific application or the most recent
...
-- 4) Helper to list all applications for an email/password
CREATE OR REPLACE FUNCTION public.candidate_list_applications(_email text, _password text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  END IF;
  SELECT password_hash INTO v_hash FROM public.candidates
    WHERE lower(email) = v_email AND password_hash IS NOT NULL LIMIT 1;
  IF v_hash IS NULL OR v_hash <> crypt(_password, v_hash) THEN
    RETURN jsonb_build_object('ok', false, 'error', 'bad_credentials');
  END IF;
  ...
  RETURN jsonb_build_object('ok', true, 'applications', v_apps);
END $function$;
