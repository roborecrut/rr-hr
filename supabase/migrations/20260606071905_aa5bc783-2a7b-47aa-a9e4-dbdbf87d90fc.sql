CREATE OR REPLACE FUNCTION public.candidate_update_profile(_token uuid, _patch jsonb DEFAULT '{}'::jsonb, _new_email text DEFAULT NULL::text, _new_password text DEFAULT NULL::text, _current_password text DEFAULT NULL::text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_norm_email text;
  v_hash text;
  v_full_name text := NULLIF(btrim(COALESCE(_patch->>'full_name', '')), '');
BEGIN
  ...
  IF v_full_name IS NOT NULL THEN
    UPDATE public.candidates SET full_name = v_full_name
    WHERE lower(email) = lower(v_cand.email) AND full_name IS DISTINCT FROM v_full_name;
  END IF;
...
GRANT EXECUTE ON FUNCTION public.candidate_update_profile(uuid, jsonb, text, text, text) TO anon, authenticated, service_role;