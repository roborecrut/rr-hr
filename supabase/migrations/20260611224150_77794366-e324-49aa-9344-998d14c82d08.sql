...
BEGIN
  ...
  IF v_role <> 'service_role'
     AND NOT (auth.uid() = v_user OR public.has_role(auth.uid(),'admin'::public.app_role)) THEN
...
CREATE OR REPLACE FUNCTION public.get_vacancy_employer_contacts(_project_id uuid)
RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
DECLARE
...
BEGIN
  SELECT e.* INTO v_emp
    ...
    JOIN public.projects p ON p.employer_id = e.id
    WHERE p.id = _project_id;
  IF v_emp.id IS NULL THEN
    RETURN jsonb_build_object('ok', false, 'error', 'no_employer');
  END IF;
  RETURN jsonb_build_object(
    'ok', true,
    'company_name', v_emp.company_name,
    'contact_name', v_emp.contact_name,
    'contact_email', v_emp.contact_email,
    'contact_tg', v_emp.contact_tg
  );
END $$;

REVOKE ALL ON FUNCTION public.get_vacancy_employer_contacts(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_vacancy_employer_contacts(uuid) TO anon, authenticated, service_role;