...
BEGIN
  ...
  IF _item = 'interview_setup' THEN
    ...
    ON CONFLICT (project_id) DO UPDATE
      SET updated_at = now(),
          created_by = COALESCE(public.interview_systems.created_by, excluded.created_by),
          source = CASE
            WHEN public.interview_systems.source IN ('spend_fixed_credit', 'spend_fixed_balance') THEN public.interview_systems.source
