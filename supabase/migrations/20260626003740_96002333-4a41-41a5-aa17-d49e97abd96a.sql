...
DECLARE
  ...
  v_new_id         uuid;
BEGIN
  SELECT j.status INTO v_status FROM public.ai_jobs AS j WHERE j.id = _job_id FOR UPDATE;
  IF v_status IS NULL THEN RAISE EXCEPTION 'ai_job_not_found'; END IF;
  IF v_status IN (
    'primary_succeeded','fallback_succeeded','cancelled',
    'timed_out','save_failed','validation_failed','fallback_failed',
    'orchestration_failed','fallback_unavailable'
  ) THEN
    RAISE EXCEPTION 'job_terminal:%', v_status;
  END IF;
  SELECT count(*) INTO v_active_same FROM public.ai_job_attempts AS a
   WHERE a.job_id = _job_id AND a.provider = _provider AND a.status = 'started';
  IF v_active_same > 0 THEN RAISE EXCEPTION 'attempt_already_active'; END IF;
  IF _provider <> 'primary' THEN
    SELECT count(*) INTO v_active_primary FROM public.ai_job_attempts AS a
     WHERE a.job_id = _job_id AND a.provider = 'primary' AND a.status = 'started';
    IF v_active_primary > 0 THEN RAISE EXCEPTION 'primary_still_active'; END IF;
  END IF;
  SELECT COALESCE(MAX(a.attempt_number), 0) + 1 INTO v_next_number
    FROM public.ai_job_attempts AS a WHERE a.job_id = _job_id;
