DROP FUNCTION IF EXISTS public._test_advance_stage_v2_debug_dump();
...
BEGIN
  ...
  -- All four numeric scores have DEFAULT 0 and the recompute trigger averages
  -- them, so set every component explicitly to control the resulting overall.
  INSERT INTO public.candidate_scores(candidate_id, resume_score, checklist_score, situations_score, interview_score, situations_feedback) VALUES
    (v_cand_a,        90, 90, 90, 90, '{"items":[]}'::jsonb),
    (v_cand_b,        30, 30, 30, 30, '{"items":[]}'::jsonb),
    (v_cand_done,     90, 90, 90, 90, '{"items":[]}'::jsonb),
    (v_cand_conflict, 90, 90, 90, 90, '{"items":[]}'::jsonb),
    (v_cand_fb,       95, 95, 95, 95, '{"items":[]}'::jsonb);

  -- v_cand_noscore: no candidate_scores row → overall_score is NULL when queried.
  ...
  -- Cleanup
  DELETE FROM public.notifications WHERE meta->>'candidate_id' IN (v_cand_a::text, v_cand_b::text, v_cand_done::text, v_cand_conflict::text, v_cand_fb::text);
...
EXCEPTION WHEN OTHERS THEN
  BEGIN DELETE FROM public.notifications WHERE meta->>'candidate_id' IN (v_cand_a::text, v_cand_b::text, v_cand_done::text, v_cand_conflict::text, v_cand_fb::text); EXCEPTION WHEN OTHERS THEN NULL; END;
