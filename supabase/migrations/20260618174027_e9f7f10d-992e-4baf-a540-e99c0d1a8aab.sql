...
BEGIN
  ...
  INSERT INTO public.projects(id, employer_id, company_id, role_name, interview_pass_score, created_at, updated_at) VALUES
    (v_proj_a, v_employer_id, v_company_id, 'role A', 75, now(), now()),
    (v_proj_b, v_employer_id, v_company_id, 'role B', 75, now(), now()),
    (v_proj_nopass, v_employer_id, v_company_id, 'role NoPass', NULL, now(), now());
  ...
  -- helper macro via inline blocks; v_ok then accumulate.
  -- Scenario 1
  ...
  v_results := v_results || jsonb_build_array(jsonb_build_object('name','success_advances_to_training','ok',v_ok,'detail',v_resp));
  ...
  v_ok := (v_resp->>'ok')='false' AND (v_resp->>'error')='pass_score_missing'
    AND (SELECT current_stage::text FROM public.candidates WHERE id = v_cand_nopass) = 'terms';
  v_results := v_results || jsonb_build_array(jsonb_build_object('name','missing_pass_score_rejected','ok',v_ok,'detail',v_resp));
  v_total := v_total+1; IF NOT v_ok THEN v_failed := v_failed+1; END IF;
